...
The UI shows thumbnail, title, uploader, stats, and a quality picker grouped into video with sound / video only / audio. Quality labels prefixed with `~` are approximate (the API only labels what it observes); unknown sizes render as "size unknown". The video grid collapses when Best already covers the only video row. A Video/MP3 toggle switches between video and audio extraction. Shared links (`/?url=<link>`) pre-fill the input.
